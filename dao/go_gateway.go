...
type ServiceAccessControl struct {
	...
	OpenAuth          int8   `gorm:"column:open_auth;type:tinyint(4);not null" json:"open_auth"`                  // 是否开启权限 1=开启
...
type ServiceGrpcRule struct {
	...
	HeaderTransfor string `gorm:"column:header_transfor;type:varchar(5000);not null" json:"header_transfor"` // header转换支持增加(add)、删除(del)、修改(edit) 格式: add headname headvalue 多个逗号间隔
}
...
type ServiceHTTPRule struct {
	gorm.Model
	ServiceID      uint   `gorm:"column:service_id;type:bigint(20);not null" json:"service_id"`              // 服务id
	RuleType       int8   `gorm:"column:rule_type;type:tinyint(4);not null" json:"rule_type"`                // 匹配类型 0=url前缀url_prefix 1=域名domain
	Rule           string `gorm:"column:rule;type:varchar(255);not null" json:"rule"`                        // type=domain表示域名，type=url_prefix时表示url前缀
	NeedHTTPs      int8   `gorm:"column:need_https;type:tinyint(4);not null" json:"need_https"`              // 支持https 1=支持
	NeedStripURI   int8   `gorm:"column:need_strip_uri;type:tinyint(4);not null" json:"need_strip_uri"`      // 启用strip_uri 1=启用
	NeedWebsocket  int8   `gorm:"column:need_websocket;type:tinyint(4);not null" json:"need_websocket"`      // 是否支持websocket 1=支持
...
type ServiceLoadBalance struct {
	...
	RoundType              int8   `gorm:"column:round_type;type:tinyint(4);not null" json:"round_type"`                          // 轮询方式 0=random 1=round-robin 2=weight_round-robin 3=ip_hash
