...
type ServiceAccessControl struct {
	...
	OpenAuth          OpenAuthType `gorm:"column:open_auth;type:tinyint(4);not null" json:"open_auth" validate:"oneof=0 1"` // 是否开启权限 1=开启
	BlackList         IpListType   `gorm:"column:black_list;type:varchar(1000);not null" json:"black_list"`                 // 黑名单ip
	WhiteList         IpListType   `gorm:"column:white_list;type:varchar(1000);not null" json:"white_list"`                 // 白名单ip
...
type ServiceGrpcRule struct {
	...
	MetadataTransform HeaderTransformType `gorm:"column:metadata_transform;type:varchar(5000);not null" json:"metadata_transform"` // header转换支持增加(add)、删除(del)、修改(edit) 格式: add headname headvalue 多个逗号间隔
}
...
type ServiceHTTPRule struct {
	gorm.Model
	ServiceID       uint                `gorm:"column:service_id;type:bigint(20);not null" json:"service_id"`                              // 服务id
	RuleType        HttpRuleType        `gorm:"column:rule_type;type:tinyint(4);not null" json:"rule_type" validate:"oneof=0 1"`           // 匹配类型 0=url前缀url_prefix 1=域名domain
...
type ServiceInfo struct {
	...
	LoadType    LoadType       `gorm:"column:load_type;type:tinyint(4);not null" json:"load_type" validate:"oneof=0 1 2"`           // 负载类型 0=http 1=tcp 2=grpc
...
type ServiceLoadBalance struct {
	...
	RoundType              RoundType      `gorm:"column:round_type;type:tinyint(4);not null" json:"round_type" validate:"oneof=0 1 2 3"` // 轮询方式 0=random 1=round-robin 2=weight_round-robin 3=ip_hash
	IPList                 IpListType     `gorm:"column:ip_list;type:varchar(2000);not null" json:"ip_list"`                             // ip列表
	WeightList             WeightListType `gorm:"column:weight_list;type:varchar(2000);not null" json:"weight_list"`                     // 权重列表
	ForbidList             IpListType     `gorm:"column:forbid_list;type:varchar(2000);not null" json:"forbid_list"`                     // 禁用ip列表
