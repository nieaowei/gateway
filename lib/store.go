...
//	return NewRediStoreWithPool(&redis.ClusterClient{
//		MaxIdle:     size,
...
func NewRediStoreWithCluster(cluster *redis.ClusterClient, keyPairs ...[]byte) (*RediStore, error) {
	rs := &RediStore{
...
// Close closes the underlying *redis.ClusterClient
...
func (s *RediStore) Delete(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	if _, err := s.ClusterClient.Del(context.Background(), s.keyPrefix+session.ID).Result(); err != nil {
		return err
	}
...
func (s *RediStore) ping() (bool, error) {
	data, err := s.ClusterClient.Ping(context.Background()).Result()
	if err != nil {
		return false, err
	}
...
func (s *RediStore) save(session *sessions.Session) error {
	...
	_, err = s.ClusterClient.Do(context.Background(), "SETEX", s.keyPrefix+session.ID, age, b).Result()
	return err
}
...
func (s *RediStore) load(session *sessions.Session) (bool, error) {
	data, err := s.ClusterClient.Get(context.Background(), s.keyPrefix+session.ID).Bytes()
	if err != nil {
		return false, err
	}
...
type redisStoreClusterCli struct {
	*RediStore
}

func (c *redisStoreClusterCli) Options(options gin_sessions.Options) {
...
func NewRedisStoreClusterCli(cluster *redis.ClusterClient, keypair ...[]byte) (gin_sessions.Store, error) {
	store, err := NewRediStoreWithCluster(cluster, keypair...)

	return &redisStoreClusterCli{store}, err
}
